...
//Create infantry
	_maxStrength = 1 + (1 max (round (_size/50)));
	_spawnPos = [];
	_groupType = "";
	_currentStrength = 0;
	if (_isFrontline) then {_maxStrength = _maxStrength + 1};

	while {(spawner getVariable _marker < 2) AND (_currentStrength < _maxStrength)} do {
