...
_tsk = ["NATOArmor",[side_blue,civilian],[format [localize "STR_TSK_TD_NATO_ARMOR",_targetName,_originName,numberToDate [2035,_endTime] select 3,numberToDate [2035,_endTime] select 4, A3_Str_BLUE],format ["%1 Armor", A3_Str_BLUE],_targetMarker],_targetPosition,"CREATED",5,true,true,"Attack"] call BIS_fnc_setTask;
...
if (({alive _x} count _allSoldiers == 0) OR ({(alive _x)} count _allVehicles == 0)) then {
	_tsk = ["NATOArmor",[side_blue,civilian],[format [localize "STR_TSK_TD_NATO_ARMOR",_targetName,_originName,numberToDate [2035,_endTime] select 3,numberToDate [2035,_endTime] select 4, A3_Str_BLUE],format ["%1 Armor", A3_Str_BLUE],_targetMarker],_targetPosition,"FAILED",5,true,true,"Attack"] call BIS_fnc_setTask;
