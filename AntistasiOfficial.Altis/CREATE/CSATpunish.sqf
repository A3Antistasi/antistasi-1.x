...
_tsk = ["AtaqueAAF",[side_blue,civilian],[["STR_TSK_CSAT_PUNISH_DESC",_nombredest],"STR_TSK_CSAT_PUNISH_TITLE",_mrkDestino],getMarkerPos _mrkDestino,"CREATED",10,true,true,"Defend"] call BIS_fnc_setTask;
...
waitUntil {sleep 5;
	...
	(time > _tiempo)};
		...
		if ((({!(captive _x)} count _soldados) < ({captive _x} count _soldados)) or ({alive _x} count _soldados < round (_solMax / 3)) or (time > _tiempo)) then {
			...
			_tsk = ["AtaqueAAF",[side_blue,civilian],[["STR_TSK_CSAT_PUNISH_DESC",_nombredest],"STR_TSK_CSAT_PUNISH_TITLE",_mrkDestino],getMarkerPos _mrkDestino,"SUCCEEDED",10,true,true,"Defend"] call BIS_fnc_setTask;
			...
			_tsk = ["AtaqueAAF",[side_blue,civilian],[["STR_TSK_CSAT_PUNISH_DESC",_nombredest],"STR_TSK_CSAT_PUNISH_TITLE",_mrkDestino],getMarkerPos _mrkDestino,"FAILED",10,true,true,"Defend"] call BIS_fnc_setTask;
			[-5,-20,_posdestino] remoteExec ["AS_fnc_changeCitySupport",2];
