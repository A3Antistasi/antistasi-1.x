...
if (_medic != _unit) then {
	...
	if (!(_unit getVariable ["ASunconscious", false])) then {
	...
	while {true} do {
		...
		if ((!alive _medic) or (!alive _unit) or (_medic distance _unit < 3) or (_timeOut < time) or (_medic getVariable "ASunconscious") or (_unit != vehicle _unit) or (_medic != vehicle _medic)) exitWith {};
		if (isPlayer _unit) then{
			if ((unitReady _medic) and (alive _medic) and (_medic distance _unit > 3) and (!(_medic getVariable "ASunconscious"))) then {_medic setPos position _unit};
		};
		sleep 1;
	};
	...
	if ((_unit distance _medic < 3) and (alive _unit) and (alive _medic)) then
		...
		_healed = true;
		if ((alive _medic) and (alive _unit) /*and (not(_unit getVariable "ASunconscious"))*/ and (not(_medic getVariable "ASunconscious"))) then {
