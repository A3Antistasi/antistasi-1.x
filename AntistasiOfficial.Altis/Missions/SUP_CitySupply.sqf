...
params ["_type", "_create", "_position"];
...
if(_create) then 
{
	...
	while {nearestObjects [_spawnPosition, ["Land_PaperBox_01_open_boxes_F", "Land_PaperBox_01_open_water_F", "CargoNet_01_barrels_F"], 300, true] count != 0} do 
	{
		_allSheds = _allSheds - [_selectedShed];
		if(_allSheds count == 0) exitWith {_spawnPosition = nil};
		_spawnPosition = position _selectedShed;
	};
