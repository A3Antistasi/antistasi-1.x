...
//Standard Equipment (good for 5 people)
			...
			_crate addWeaponCargoGlobal 		[selectrandom bluRifle		,	5	];
...
// Additional equipment depending on ArmyLevel
	if (BE_currentStage == 3) then {
			...
			_crate addMagazineCargoGlobal 		[selectrandom bluRifleAmmo	,	50	];
	}
	else {
	if (BE_currentStage == 2) then {
			...
			_crate addMagazineCargoGlobal 		[selectrandom bluRifleAmmo	,	25	];
	}
	else {
