...
switch (_grp getvariable "UPSMON_GrpMission") do
{
            ...
            If (!(_grp  getvariable ["UPSMON_GrpinAction",false])) then
            {
                If (_targetdist <= 300) then
                {
                    If (IsNull _target) then
                    {
                        ...
                        {
                            [_grp,_grp getvariable ["UPSMON_attackpos",_currpos],_currpos] call UPSMON_GETINPATROLSRCH;
                        };
                    }
                    else
                    {
            ...
            If (_grp getvariable ["UPSMON_SRCHTIME",time] > time) then
            {
                if (!(_grp getvariable ["UPSMON_searchingpos",false])) then
                {
                    if (!(_grp getvariable ["UPSMON_Disembarking",false])) then
                    {
                        ...
                        {
                            [_grp,_grp getvariable ["UPSMON_Alertpos", _currPos],_typeofgrp,_areamarker] spawn UPSMON_DOPATROLSRCH;
                        };
                    };
                };
            }
    ...
    case "PATROLINBLD":
    {
        If (_targetdist <= 100) then
        {
            ...
            {
                {_x forceSpeed -1; _x doMove getPos _x;}count units _grp;
                If (!(_grp getvariable ["UPSMON_InBuilding",false])) then
                {
                    [units _grp, _grp getvariable ["UPSMON_bldposToCheck",[]], _grp, time+300] spawn UPSMON_patrolBuilding;
                };
            }
            else
            {
                [_grp, _currpos getPos [100, random 360], _currpos] call UPSMON_GETINPATROLSRCH;
            };
        };
    };
