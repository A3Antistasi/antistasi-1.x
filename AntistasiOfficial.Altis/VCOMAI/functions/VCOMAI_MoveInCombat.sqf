...
if (VCOM_CurrentlyMoving < VCOM_CurrentlyMovingLimit) then
{
	...
	if (_VCOM_MovedRecentlyCover || {(diag_tickTime - _FiredRecently) < 3} || {_VCOM_VisuallyCanSee} || {_VCOMAI_ActivelyClearing} || {_VCOMAI_StartedInside} || {_VCOM_GARRISONED} || {_VCOM_MovedRecently}) exitWith {};
