...
_deletedead = ["DELETE:",0,_UCthis] call UPSMON_getArg;
{
	...
	{//soldiers
		_x AddEventHandler ["hit", {_this spawn UPSMON_SN_EHHIT}];
		sleep 0.05;
		_x AddEventHandler ["killed", {_this spawn UPSMON_SN_EHKILLED}];
		//_x AddEventHandler ["fired", {_this spawn UPSMON_SN_EHFIRED}];
	}
	else
	{//civ
		if (!isnil "_x") then
		{
			sleep 0.05;
			_x AddEventHandler ["firedNear", {_this spawn UPSMON_SN_EHFIREDNEAR}];
			sleep 0.05;
			_x AddEventHandler ["killed", {_this spawn UPSMON_SN_EHKILLEDCIV}];
			sleep 0.05;
		};
	};
